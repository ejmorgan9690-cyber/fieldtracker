...
export default function LoggingForm() {
  ...
  const startVoiceRecognition = () => {
    ...
    recognition.onresult = (event) => {
      ...
      // Aggressively map numbers and homophones to digits in real-time
      const numMap = { 
        'one': '1', 'won': '1',
        'two': '2', 'to': '2', 'too': '2',
        'three': '3', 'tree': '3',
      ...
      Object.keys(numMap).forEach(k => {
        ...
        newPhrases = newPhrases.replace(regex, numMap[k]);
      });

      // Auto-correct construction terminology
      newPhrases = newPhrases.replace(/\bboard\b/gi, 'bore')
                             ...
                             .replace(/\bcloud\b/gi, 'plow')
                             .replace(/\bduck\b/gi, 'duct')
                             .replace(/\bducks\b/gi, 'duct')
                             .replace(/\bdot\b/gi, 'duct')
                             .replace(/\bdoc\b/gi, 'duct')
                             .replace(/\bdock\b/gi, 'duct')
                             .replace(/\bdust\b/gi, 'duct')
                             .replace(/\bfibre\b/gi, 'fiber')
                             .replace(/\bprop\b/gi, 'drop')
                             .replace(/\broot\b/gi, 'route')
                             .replace(/\bfrench\b/gi, 'trench')
